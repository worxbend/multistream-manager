...
| T26 | Helix polling: stream live/viewers (60s), followers/subs (120s) | not ported — the host Dashboard already polls exactly these via its own `twitch.rs` backend; duplicating the pollers per chat account would double API traffic for data already on screen. The *events* behind those totals are a different question, answered by `eventsub.rs`: a follower **count** is dashboard furniture, a follower **arriving** is news, and polling could never tell you about the second at the moment it happened |
| T36 | Events that never reach chat (beyond twi) | ported → `eventsub.rs` — EventSub over WebSocket for follows, channel-point redemptions, hype trains, polls and predictions. Deliberately does *not* subscribe to subs, gifts, cheers or raids: all four arrive over IRC already, and two notifications for one event is worse than one |
...
| T29 | Desktop notifications | ported → `notify.rs` (delivery: notify-send → gdbus → kdialog → bell) + `chat/notify.rs` (which events qualify). Windows toast omitted — documented deviation |
| T35 | Moderation and raids (beyond twi, which performs none) | ported → `chat/twitch.rs::run_moderation` — Helix `moderation/chat`, `moderation/bans` and `raids`, since Twitch removed these from IRC in 2023. twi leaves them to the Twitch dashboard; msm moderates YouTube from the same keys, and one platform answering a key that the other refuses was the asymmetry worth closing |
